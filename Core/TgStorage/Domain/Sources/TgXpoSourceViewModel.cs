...
public sealed partial class TgXpoSourceViewModel : TgViewModelBase
{
    #region Public and private fields, properties, constructor

    public TgXpoSourceEntity Item { get; set; }
	...
	public Guid SourceUid
	{
		...
		set
		{
			TgXpoOperResult<TgXpoSourceEntity> operResult = XpoContext.SourceRepository.GetAsync(value).GetAwaiter().GetResult();
			Item = operResult.IsExist
				? operResult.Item
				: XpoContext.SourceRepository.GetNewAsync().GetAwaiter().GetResult().Item;
		}
	}
    ...
    public TgXpoSourceViewModel(TgXpoSourceEntity item)
    {
		Item = item;
    ...
    public TgXpoSourceViewModel(TgXpoContext xpoContext) : this(xpoContext.SourceRepository.CreateNewAsync().GetAwaiter().GetResult().Item) { }
