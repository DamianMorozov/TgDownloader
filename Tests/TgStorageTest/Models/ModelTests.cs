...
internal class ModelTests
{
	...
	public void TgStorage_Filter_Constructor()
	{
		Assert.DoesNotThrow(() =>
		{
			...
			Assert.Multiple(() =>
			{
				...
				Assert.That(Equals(TgFilterType.None, filter.FilterType));
				Assert.That(Equals("Any", filter.Name));
				Assert.That(Equals("*", filter.Mask));
				Assert.That(Equals((long)0, filter.Size));
				Assert.That(Equals(TgFileSizeType.Bytes, filter.SizeType));
			});
		});
	}
	...
	public void TgStorage_Message_Constructor()
	{
		Assert.DoesNotThrow(() =>
		{
			...
			Assert.Multiple(() =>
			{
				...
				Assert.That(Equals(TgMessageType.Message, message.Type));
	...
	public void TgStorage_Proxy_Constructor()
	{
		Assert.DoesNotThrow(() =>
		{
			...
			Assert.Multiple(() =>
			{
				Assert.That(Equals(TgProxyType.None, proxy.Type));
