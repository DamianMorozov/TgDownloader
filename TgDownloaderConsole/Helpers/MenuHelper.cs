...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	...
	internal void ShowTableMain(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMain, 
		FillTableColumns, FillTableRowsMain);

	internal void ShowTableStorageSettings(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainStorage, 
		FillTableColumns, FillTableRowsStorage);

	internal void ShowTableFiltersSettings(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainFilters, 
		FillTableColumns, FillTableRowsFilters);

	internal void ShowTableAppSettings(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.TgSettings, 
		FillTableColumns, FillTableRowsApp);

	internal void ShowTableClient(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.TgSettings, 
		FillTableColumns, FillTableRowsClient);

	internal void ShowTableDownload(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainDownload, 
		FillTableColumns, FillTableRowsDownload);

	internal void ShowTableAdvanced(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainDownload, 
		FillTableColumns, FillTableRowsAdvanced);

	internal void ShowTableScanSources(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainDownload, 
		FillTableColumns, FillTableRowsScanSources);

	internal void ShowTableViewSources(TgDownloadSettingsModel tgDownloadSettings) => ShowTableCore(tgDownloadSettings, TgLocale.MenuMainDownload, 
		FillTableColumns, FillTableRowsViewSources);
	...
	internal void FillTableRowsSource(TgDownloadSettingsModel tgDownloadSettings, Table table)
	{
		...
		else
		{
			...
			if (!string.IsNullOrEmpty(tgDownloadSettings.SourceTitle))
				sourceValue += $" | {tgDownloadSettings.SourceTitle}";
	...
	private bool AskQuestionReturnPositive(string question, bool isTrueFirst) =>
		AnsiConsole.Prompt(new SelectionPrompt<bool>()
			.Title($"{question}?")
			.PageSize(3)
			.AddChoices(isTrueFirst ? new List<bool> { true, false } : new List<bool> { false, true }));

	private bool AskQuestionReturnNegative(string question, bool isTrueFirst = false) =>
