...
public sealed class TgSqlFilterHelper : TgSqlHelperBase<TgSqlTableFilterModel>
{
	#region Design pattern "Lazy Singleton"

#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	private static TgSqlFilterHelper _instance;
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	public static TgSqlFilterHelper Instance => LazyInitializer.EnsureInitialized(ref _instance);
	...
	public override TgSqlTableFilterModel NewItem() =>
		new() { IsEnabled = true, FilterType = TgFilterType.SingleName, Name = "Any", Mask = "*", SizeType = TgFileSizeType.Bytes };

	public override TgSqlTableFilterModel NewItem(Session session) =>
		new(session) { IsEnabled = true, FilterType = TgFilterType.SingleName, Name = "Any", Mask = "*", SizeType = TgFileSizeType.Bytes };

	public TgSqlTableFilterModel GetItem(TgFilterType filterType, string name) =>
		new UnitOfWork()
	...
	public override bool AddItem(TgSqlTableFilterModel item)
	{
		...
		TgSqlTableFilterModel itemNew = new(uow)
		{
			...
			Name = item.Name,
			Mask = string.IsNullOrEmpty(item.Mask) && (Equals(item.FilterType, TgFilterType.MinSize) || Equals(item.FilterType, TgFilterType.MaxSize)) ? "*" : item.Mask,
			Size = item.Size,
	...
	public override bool IsValidXpLite(TgSqlTableFilterModel item) => new TgSqlTableFilterValidator().Validate(item).IsValid;
	...
	public bool DeleteDefaultItem()
	{
		...
		List<TgSqlTableFilterModel> filters = GetList(false);
